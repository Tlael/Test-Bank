...
class AdminSteps(BaseSteps):
    ...
    def create_credit_user(self, create_user_request: CreateUserRequest) -> CreateCreditUserRequest:
        credit_user_request = CreateCreditUserRequest(
            username=f"C{create_user_request.username[:14]}",
            password=create_user_request.password,
            role="ROLE_CREDIT_SECRET"
        )

        self.create_user(credit_user_request)
